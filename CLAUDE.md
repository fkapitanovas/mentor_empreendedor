...
- System prompt modular (15 blocos + 2 dinâmicos): Identidade/Tom, Base de Conhecimento (13 gurus incl. Ana Fontes e Alfredo Soares), Base de Livros (23 livros, curadoria v1.2 — foco em MEI BR), Regras de Interação, Personalização, Resolução de Conflitos (9 tensões), Referências Nicho (26 influenciadores), Base Institucional (Sebrae + gov.br + Guia Definitivo do MEI Sebrae-SC), Base Impulso Stone (8 módulos), Formalização MEI, E-commerce/Marketplaces, **Vendas (bloco dedicado, 10 seções: CHA/SPIN/objeções/11+13 estratégias Insta+WhatsApp/follow-up/pós-venda)**, Ferramentas Práticas, Diagnóstico/Atualização de Perfil. **Total ~70.000 tokens (~35% do contexto 200k do Sonnet 4.6).**
...
- **Input de áudio (Groq Whisper Turbo)**: botão de microfone no chat-input (entre textarea e send) com 3 estados visuais (idle/recording/transcribing). Gravação no browser via MediaRecorder (WebM/Opus, fallback MP4), envio multipart pra `/api/transcribe` (Node runtime, valida sessão Supabase), Groq `whisper-large-v3-turbo` com `language='pt'`, retorna texto que popula/anexa ao textarea pra revisão antes do envio. Cap 2min, min 1s, max 10MB, zero storage de áudio. Feature flag `NEXT_PUBLIC_VOICE_ENABLED=true` + `GROQ_API_KEY` em Vercel production. Logs estruturados sem texto transcrito (privacy). Custo ~$0.0007/min no Groq free tier.
